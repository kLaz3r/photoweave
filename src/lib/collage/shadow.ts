...
export function drawImageWithShadow(
  ...
  // Clip to rounded rect for softer corners on the shadow
  ...
  // Draw a straight-cornered white border as a contrast ring around the image.
  // A stroke centered on the image edge has its inner half covered by the image
  // and its outer half visible — giving clean straight corners.
  ctx.strokeStyle = "white";
  ctx.lineWidth = borderThickness;
  // Draw a plain rect path at the image boundary; stroke is centered on all edges
  ctx.beginPath();
  _rect(ctx, x, y, w, h);
  ctx.stroke();
